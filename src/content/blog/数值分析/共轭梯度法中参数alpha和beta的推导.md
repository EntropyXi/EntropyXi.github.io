...
categories:
  - "数值分析"
permalink: "2026/02/08/数值分析/共轭梯度法中参数alpha和beta的推导"
math: true
draft: false
---
...
这意味着我们要选择一个 $\alpha_k$，使得目标函数 $\varphi$ 在这条直线上达到**最小值**。

我们将 $\mathbf{x}_{k+1} = \mathbf{x}_k + \alpha \mathbf{p}_k$ 代入 $\varphi(\mathbf{u})$ 的表达式中，把 $\varphi$ 看作只关于 $\alpha$ 的一元函数 $f(\alpha)$：
...
回顾 $\mathbf{p}_k$ 的定义：$\mathbf{p}_k = \mathbf{r}_k + \beta_{k-1} \mathbf{p}_{k-1}$。两边同时左乘 $\mathbf{r}_k^T$：
...
回顾残差的更新公式：$\mathbf{r}_{k+1} = \mathbf{r}_k - \alpha_k \mathbf{A} \mathbf{p}_k$。我们可以反解出 $\mathbf{A} \mathbf{p}_k$：
...
&= \mathbf{r}_{k+1}^T \left[ -\frac{1}{\alpha_k} (\mathbf{r}_{k+1} - \mathbf{r}_k) \right] \\
...
因为新残差与旧残差正交（$\mathbf{r}_{k+1}^T \mathbf{r}_k = 0$）。

现在看分母，回顾 $\alpha_k = \frac{\mathbf{r}_k^T \mathbf{r}_k}{\mathbf{p}_k^T \mathbf{A} \mathbf{p}_k}$，所以分母等于：
