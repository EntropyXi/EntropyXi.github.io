...
5. **所有外部 Agent 产出必须由技术负责人（agy Claude）审查**：外部 Agent 没有最终放行权；未完成技术负责人的差异审查、测试和浏览器验收，不得视为完成。
...
出现以下任一情况必须先写 ADR，技术负责人放行后实施；这里是“允许优化的决策门”，不是永久冻结：
...
- 目标规模通常 100–400 行人工变更；超过约 800 行或跨 15 个以上文件时，总调度（Codex）必须先拆分。若因原子迁移确实无法拆分，必须在变更说明中记录不可拆理由、分段 review 顺序和额外回归范围，未记录则不进入 review。
- 纯重命名/搬迁与功能修改分开提交。
...
- `agy` 的首次启动按用户要求从 PowerShell 发起，固定入口为 `C:\Users\entropy\AppData\Local\agy\bin\agy.exe`；该步骤之后，本项目由技术负责人、DSH 和 Antigravity 发起的全部项目终端命令必须在 Git Bash 中执行。Antigravity CLI 的交互输入继续写入其已启动会话，不视为另开 PowerShell 项目命令；Read/Glob/Grep、浏览器检查和非终端文件补丁不属于终端命令。若某审查执行器无法选择 Git Bash，则该会话禁止使用其 Shell/Pwsh 工具，由技术负责人在 Git Bash 中完成所需核验。
...
9. 技术负责人给出“通过/需修改/拒绝”结论；
10. 若影响生产路径，回滚点和 smoke check 已记录。
11. 对应的产物审计、URL/HTML/资源检查已通过；主要可视批次的 §12.5 截图索引已由总调度（Codex）触发采集并由技术负责人记录。

### 8.3 审查标准

技术负责人按 Google Engineering Practices 的维度审查：设计、功能、复杂度、测试、命名、注释、风格、文档和代码健康。
...
### 9.1 技术负责人（agy Claude）与总调度（Codex）

> **治理冲突解析**：本节（§9.1–§9.5）原文使用"主 Agent"统称调度与技术执行职责。依据用户最新治理契约（§9.6），该职责现拆分为两个独立角色：**总调度（Codex）** 负责任务拆分、模型路由、进度协调、权限/租约协调、触发验收门；**技术负责人（agy Claude，Claude Sonnet 4.6 Thinking）** 负责架构边界、复杂/关键核心代码、测试与 CI、技术验收条件制定、逐文件技术 review、可复现缺陷裁决和 APPROVED。两者永远在独立进程/上下文中运行。§9.6 与本节原文冲突处以 §9.6 为准，且已在本节直接消除，不再依赖覆盖条款。
...
- 接收用户需求，将其拆分为有界任务单元；
- 决定将任务路由给哪个执行进程（技术负责人 / Gemini / DeepSeek）；
- 跟踪进度状态、维护权限白名单和文件租约协调；
- 在技术负责人完成 APPROVED 后触发阶段验收门，向用户汇报。
...
- 维护本计划架构边界、依赖顺序和风险登记（任务拆分与模型路由由总调度 Codex 负责）；
- 编写复杂逻辑、关键核心代码和跨组件基础设施；
- 设计动效生命周期、状态机、性能策略、测试策略和回滚方案；
- 向总调度（Codex）提供每批任务的技术范围与约束（技术输入、技术禁止项、技术验收条件、测试命令）；文件白名单和文件租约由 Codex 依据上述约束下发；

- 对所有执行进程产出做逐文件 diff review、浏览器 review 和测试；
- 对 `blocking/major` 问题要求返工，最终只由技术负责人（agy Claude）签发 APPROVED。

技术负责人禁止事项：

- 不直接编写或优化主要前端可视界面；此类任务必须派给前端可视实现者（Antigravity/Gemini）；
- 不因执行进程声称"测试通过"就跳过本地复核；
- 不把复杂核心逻辑下放后完全失去所有权；
- 不承担任务拆分、模型路由、文件租约下发或权限白名单维护（归总调度 Codex）；
- 不允许两个执行进程同时写同一文件或相互覆盖未审查变更。
...
- `Read Only` 表示不得修改源码、文档、配置、锁文件和其他版本控制内容；审查会话不负责运行会写入 `dist/`、缓存或截图的测试。需要构建或测试时，由技术负责人（agy Claude）在 Git Bash 中复跑并把结果作为审查输入。

允许任务：

- 简单逻辑、纯函数、普通编码、测试夹具、审计脚本和机械性文档更新；
- 对计划、架构、测试、性能和安全进行独立审查；
- 分析技术负责人提供的有限 diff，提出问题和测试建议。
...
1. 总调度（Codex）下发任务书，技术负责人（agy Claude）提供技术验收条件；
2. 执行进程报告修改文件、设计说明、测试和未决问题；
3. 技术负责人（agy Claude）检查 diff、运行相关测试、必要时要求修改；
4. 只有技术负责人（agy Claude）明确记录 `APPROVED` 才进入下一任务。
...
- 所有"主要且涉及前端可视界面"的编写与优化必须派给该执行进程，不能由技术负责人（agy Claude）或 DeepSeek 直接完成；
- 包括 Astro 可视组件模板、CSS、设计令牌的视觉值、响应式布局、Hero、导航外观、卡片、装饰 SVG、动效表现参数和视觉微调；
- 也可以承担简单逻辑或普通编码，但这是次要用途。

技术负责人（agy Claude）仍保留：

- 组件契约、目录与依赖方向；
- 动效控制器、生命周期、安全和性能核心逻辑；
- 测试门禁、代码审查、浏览器验收和最终放行。
- 为消除写租约冲突，阶段 2 的结构性脚本拆分属于技术负责人（agy Claude）基础设施例外：技术负责人（agy Claude）只可修改相关 `.astro` 的 `<script>` 块和最薄初始化桥接，不改可视标记、样式或视觉参数；完成行为等价回归并释放租约后，组件才交给 Antigravity。

前端可视实现者（Antigravity/Gemini）交付必须包含：

- 改动前目标与参考特征；
- 修改文件清单；
- 桌面/移动、暗色/亮色、reduced-motion 的视觉断言清单，以及其能力范围内可提供的截图；§12.5 固定截图由总调度（Codex）触发采集，技术负责人（agy Claude）核验内容并署名，不能用执行进程的自证代替；
- 未使用参考站受限素材的声明；
...
- 总调度（Codex）派工时依据技术负责人提供的技术范围下发文件白名单（允许修改）和只读参考清单。
- 执行进程完成后停止写入，技术负责人（agy Claude）审查期间其他执行进程不得改这些文件。
- 需要返工时重新授予租约；禁止边审查边并发重写。
- 若出现意外重叠，立即停止相关执行进程，以 `git diff` 为证据人工合并，不接受整文件覆盖。
- 每批外部产出先存为未提交工作树变更；技术负责人（agy Claude）签发 APPROVED 前不提交。

### 9.5 派工决策表

| 任务                    | 总调度（Codex）    | 技术负责人（agy Claude）                      | DeepSeek 子 Agent | Antigravity 子 Agent |
| ----------------------- | ------------------ | --------------------------------------------- | ----------------- | -------------------- |
| 架构、ADR、阶段拆分     | 负责任务拆分与路由 | 主责                                          | 审查              | 提供可视实现反馈     |
| 复杂动效生命周期/状态机 | 不负责             | 主责                                          | 可审查            | 对接并实现视觉层     |
| 主要 Astro UI/CSS/SVG   | 不负责             | 禁止直接实现，仅负责技术契约与事实缺陷 review | 禁止              | 强制主责             |
| 简单纯函数/审计脚本     | 不负责             | 可做                                          | 可主责            | 可做（次要）         |
| 单元/E2E/性能门禁       | 触发验收门         | 主责                                          | 可协助普通用例    | 补视觉行为夹具       |
| 浏览器视觉调优          | 不负责             | 监督与最终验收                                | 不负责            | 主责                 |
| 最终 diff review        | 协调权限与租约     | 唯一主责                                      | 可提供第二意见    | 自检但无放行权       |

### 9.6 治理契约（锁定规则，本会话后续每个任务均继承）

本节将已在本会话开始时确认的身份契约写成正式版本控制规则，与上述 §9.1–§9.5 具有同等约束力。任何违反本节规则的输出或操作均视为越界，应输出 `OUT_OF_SCOPE` 并停止执行。

#### 9.6.1 角色固定

| 角色标识                                                                | 绑定模型/进程                            | 职责边界                                                                                                      | 禁止事项                                                                                   |
| ----------------------------------------------------------------------- | ---------------------------------------- | ------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------ |
| **技术负责人（agy Claude；额度耗尽时由 agy Gemini 3.1 Pro High 接管）** | 独立 agy 进程                            | 架构、复杂逻辑、关键核心代码、测试与 CI、技术审查、缺陷裁决、阶段/发布 APPROVED                               | 不得修改或指导布局、样式、动画参数、视觉资产；不得给审美方案；不得与 Gemini 进程共享上下文 |
| **总调度（Codex）**                                                     | Codex 系统调度                           | 任务拆分、模型路由、进度跟踪、权限白名单、文件租约协调、触发验收门                                            | 不做架构决策、不写代码、无技术审查权、无 APPROVED 放行权                                   |
| **前端可视实现者（第二子 Agent）**                                      | Gemini 3.7 Flash (High)，独立 agy 进程   | 所有"主要且涉及前端可视界面"的编写与优化：Astro 可视组件模板、CSS、设计令牌视觉值、响应式布局、动效表现参数等 | 无最终放行权；不得修改核心逻辑、生命周期或安全策略；不得与 Claude 进程共享上下文           |
| **普通编码与双高精度审查者**                                            | DeepSeek-V4-Flash Max，dsh --profile web | 简单逻辑、纯函数、普通编码、独立审查（Read Only 模式）                                                        | 不得实现主要前端可视界面；其代码必须经技术负责人 review 后方可采纳；不得自行宣布通过       |

#### 9.6.2 结构化交接与两个独立 agy 进程规则

- **Claude 与 Gemini 永远在独立进程、独立上下文中运行，绝不在同一 agy 进程或同一对话中切换模型**。
- 技术负责人（Claude）的 agy 会话和前端可视实现者（Gemini）的 agy 会话是两个物理隔离的进程，不得互相传递对方的原始上下文，也不得让一方直接"看到"另一方的工具调用历史。
- 角色间的信息传递（如任务拆分结果、diff 摘要、审查结论等）只通过总调度（Codex）协调，或以结构化文档记录，绝不通过共享上下文。
- 任务结束后，总调度（Codex）依据技术负责人的 APPROVED 结论，推进整体计划并向用户汇报。

#### 9.6.3 Claude 额度耗尽后的接管规则

- **仅当 agy 明确显示 Claude 额度用尽时**，才触发接管流程；卡顿、超时、输出截断或任何其他失败均不算额度耗尽，不得触发接管，须输出 `CLAUDE_QUOTA_EXHAUSTED` 并由总调度（Codex）确认。
- 接管模型固定为 **Gemini 3.1 Pro (High)**，在独立 agy 进程中承担与技术负责人完全相同的身份和职责边界。
- 接管时，总调度（Codex）负责将当前技术负责人的上下文摘要（任务书、已批准决策、当前阶段状态）以书面形式传递给接管进程，不得使两进程共享原始上下文。
- 额度耗尽事件须记录：触发时间、当时任务状态、已完成工作和待处理事项，写入阶段执行记录。
- 额度恢复后，优先由 Claude Sonnet 4.6 (Thinking) 重新接管，接管方式相同。

#### 9.6.4 DeepSeek 双高精度审查规则

- DeepSeek-V4-Flash Max 的审查必须以两次**完全独立**的会话执行，每次会话开始时均为全新上下文，不得参考另一次会话的结论。
- 审查会话权限固定为 **Read Only**；不得在审查会话中修改源码、文档、配置或锁文件。
- 需要构建或测试时，由技术负责人在 Git Bash 中复跑并把结果作为审查输入，审查会话本身不运行会写入 `dist/`、缓存或截图的命令。
- 审查结论须标注严重级别（blocking / major / minor / nit）和判断依据；技术负责人逐项裁决并记录吸收/拒绝理由；DeepSeek 无放行权。
- DeepSeek 产出的代码（非只读审查产物）必须经技术负责人 diff review 后才可采纳，不得以"审查者自己写的"为由绕过 review。

#### 9.6.5 终端工具约束

- **PowerShell 唯一允许的用途**：启动 agy 进程（绝对路径 `C:\Users\entropy\AppData\Local\agy\bin\agy.exe`）。
- **项目内的一切终端命令**（构建、测试、Git 操作、审计脚本、格式化、linting 等）**必须在 Git Bash 中执行**，无论由哪个 Agent 发起。
- 若某审查执行器无法选择 Git Bash，则该会话**禁止**使用其 Shell/Pwsh 工具；所需核验改由技术负责人在 Git Bash 中完成。
- Read/Glob/Grep、浏览器检查和非终端文件补丁不属于终端命令，不受上述约束。
- Antigravity CLI 的交互输入继续写入其已启动会话，不视为另开 PowerShell 项目命令。

#### 9.6.6 边界自检与越界处理

- 每次任务开始前，技术负责人必须先执行**边界自检**：确认任务是否属于架构、复杂逻辑、核心代码、测试或审查范畴。
- 若任务明确属于"主要且涉及可视前端或动画的实现"，输出 `OUT_OF_SCOPE`，将任务转派给独立进程中的 Gemini。
- 若发现可视缺陷或计划缺失，以 `GEMINI_RETURN_LIST` 格式输出，每项包含：路径、环境、步骤、预期、实际、证据、严重级别。
- 审查输出格式固定为：`SCOPE / EVIDENCE / DEFECTS / NON_VISUAL_CHANGES / GEMINI_RETURN_LIST / GATE_DECISION`。

## 10. 分阶段实施计划

> **注意**：阶段 0-3（含 0.5）以及后续执行结果中的“主 Agent”及相关表述均保留当时旧治理称谓，仅作历史记录，不定义当前职责。自阶段 4A 起，任务拆分由总调度（Codex）负责，技术审批与最终放行由技术负责人负责，适用 §9.6 最新治理契约。
...
- [x] 实现原创背景层：网格、扫描线、局部光晕、低密度 SVG 流线；每层可独立关闭。
- [x] 重构固定/粘性页头、品牌字标、活动项、滚动压缩状态和焦点态。
- [x] 完成移动抽屉开合视觉、焦点管理契约和无 JS 导航兜底。
- [x] 统一主题 token，防止 FOUC；亮色保持足够对比。
- [x] 页脚采用轻量系统状态风格，不虚构实时数据。
- [x] 增加 SkipLink 和稳定的主内容焦点入口。
...
执行结果（2026-08-22）：Antigravity（Gemini 3.7 Flash / High）在阶段 3 可视租约内实现设计令牌、四层原创环境背景、页头/页脚/主题控件与全局动效样式；主 Agent实现滚动页头状态纯逻辑与薄初始化桥接，并补齐 SkipLink、背景层、滚动状态、移动/低动态/无 JS 回归测试。Chrome 实机审查发现初版负层级背景不可见，axe 又发现浅色次要文字对比度仅 4.34:1，均退回 Antigravity 自主返修。用户收紧治理边界后，所有曾受主 Agent具体前端实现建议影响的可视部分完成“返厂重修”，主 Agent只以失败事实监督。DeepSeek‑V4‑Flash / Max 两轮全新、独立、Read Only 高精度审查均无 P0/P1；有效问题（无 JS 死控件、令牌/规格偏差、低动态重复规则、裸动效参数、当前页与抽屉语义）全部关闭，错误的 SVG `stroke` 判断由 Chrome 151 计算样式和截图证伪。固定尺寸证据已保存到 `audit-screenshots/phase-3/`，5 种情形均无横向溢出、背景不捕获指针，移动端关闭扫描线与流线，低动态模式停止循环；复杂公式文章浏览器实测可见且无页面溢出。最终 `npm run check` 全绿：30 篇内容、24 项单测、33 项 E2E、5,326/24,576 B gzip 客户端预算、81 个兼容页面、30 篇文章与 1,289/1,289 条可访问公式全部通过。阶段 3 由主 Agent正式放行并独立提交。
...
### 阶段 6：文章阅读体验（Antigravity 负责可视层，技术负责人负责核心逻辑）
...
### 阶段 7：高级动效与磁吸增强（技术负责人核心，Antigravity 可视实现）
...
| ------------------ | ------------------- | ---------------------------------------- | ------------------------ | -------------------------------------------- |
| `BaseLayout`       | 保留 SEO/主题初始化 | 加 SkipLink、全局动效契约                | Antigravity 只做可视标记 | 技术负责人（agy Claude）                     |
| `CyberGrid`        | 评估演进/替换       | 多层可关闭 AmbientBackground             | Antigravity              | 技术负责人（agy Claude）                     |
| `SiteHeader`       | 重构                | 固定深色、荧光活动项、滚动态、移动抽屉   | Antigravity              | 技术负责人（agy Claude）                     |
| `ThemeToggle`      | 优化                | 无 FOUC、清晰状态、键盘与 reduced-motion | Antigravity 外观         | 技术负责人（agy Claude）逻辑                 |
| `Hero`             | 重点重构            | 全屏叙事、原创数学 SVG、分段入场         | Antigravity              | 技术负责人（agy Claude）                     |
| `PostCard`         | 重点重构            | 视觉分层、分类/标签/日期、焦点联动       | Antigravity              | 技术负责人（agy Claude）                     |
| `PostList`         | 调整组合            | 响应式网格、稳定列表语义                 | Antigravity              | 技术负责人（agy Claude）                     |
| `PaginationNav`    | 视觉收敛            | 静态链接、状态一致                       | Antigravity              | 技术负责人（agy Claude）                     |
| `PostLayout`       | 分步重构            | 阅读优先的标题/正文/TOC                  | Antigravity 外观         | 技术负责人（agy Claude）逻辑                 |
| `ReadingProgress`  | 核心重构            | 单 rAF、正确比例、可禁用                 | Antigravity 外观         | 技术负责人（agy Claude）                     |
| `FloatingControls` | 优化                | 不遮挡、触控友好、键盘完整               | Antigravity 外观         | 技术负责人（agy Claude）逻辑                 |
| Search             | 状态完善            | 加载/无结果/错误/结果一致                | Antigravity 外观         | 技术负责人（agy Claude） + DeepSeek 普通逻辑 |
| 聚合页面           | 视觉统一            | 时间线/面板/空状态                       | Antigravity              | 技术负责人（agy Claude）                     |
...
| 子 Agent 覆盖彼此改动        |   中 |   高 | 单文件租约、串行写入、技术负责人 diff review      | 是         |
| Agent 产出看似可用但测试不足 |   中 |   高 | 技术负责人复跑测试、浏览器验收、无自证放行        | 是         |
| 技术负责人与总调度成为瓶颈   |   中 |   中 | 小 CL、固定模板、批次抽查与阶段级全矩阵分层       | 否         |
...
> **注意**：本节审查记录系旧治理时期产物，其中出现的“主 Agent”保留当时旧治理称谓，仅作历史记录，不定义当前职责。当前技术审查裁决职责由“技术负责人”承担。
