...
- [ ] 实现 catch-all 文章静态路由。
- [ ] 实现 `PostLayout`、标题、日期、分类、标签与正文排版。
- [ ] 集成阶段 1 已由 ADR 选定的数学方案，并通过完整公式夹具。
- [ ] 实现 TOC 数据、桌面 sticky 目录和移动抽屉。
- [ ] 实现代码高亮、语言标签、复制按钮和横向滚动。
- [ ] 实现图片基础样式、灯箱的必要性评估和无 JS 降级。
- [ ] 迁移 Giscus，保持 pathname。
- [ ] 实现上一篇/下一篇。
- [ ] 对 30 篇文章运行输出审计。
- [ ] 对 5 篇复杂公式文章运行视觉回归。
