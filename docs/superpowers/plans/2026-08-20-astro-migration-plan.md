...
- [x] 定义设计 token 和 light/dark 主题。
- [x] 建立 Ayeez 参考页面的功能/视觉矩阵，区分可借鉴模式与不可复制的品牌、素材和内容。
- [x] 实现固定页头、桌面导航与移动菜单。
- [x] 实现首页 Hero、品牌文案、SVG/CSS 流光与装饰线。
- [x] 实现公告卡片和文章卡片网格。
- [x] 实现滚动显现、hover/focus 状态与 reduced-motion 版本。
- [x] 实现阅读进度、回顶和目录浮动按钮。
- [x] 评估并实现可选阅读模式。
- [x] 实现主题切换无闪烁。
- [x] 完成移动端、触控和键盘细节。
- [x] 运行 Lighthouse、可访问性和视觉回归。
- [x] Lighthouse CI、axe 和视觉基线均按 §3.6、§3.7、§11.4、§11.5 的量化标准通过。
