...
const OUTPUT_DIR = path.resolve("audit-screenshots/phase-8");
...
async function main(): Promise<void> {
  ...
  try {
    for (const testCase of cases) {
      ...
      await context.addInitScript(
        ({ theme, featureReveal, featureAmbient, featureMagnetic }) => {
          try {
            if (theme) localStorage.setItem("entropyxi-theme", theme);
            if (featureReveal !== undefined) {
              localStorage.setItem(
                "entropyxi-feature-reveal",
                String(featureReveal),
              );
            }
            if (featureAmbient !== undefined) {
              localStorage.setItem(
                "entropyxi-feature-ambient",
                String(featureAmbient),
              );
            }
            if (featureMagnetic !== undefined) {
              localStorage.setItem(
                "entropyxi-feature-magnetic",
      ...
      if (testCase.featureReveal !== undefined) {
        targetUrl.searchParams.set("feature-reveal", String(testCase.featureReveal));
      }
      if (testCase.featureAmbient !== undefined) {
        targetUrl.searchParams.set("feature-ambient", String(testCase.featureAmbient));
      }
      if (testCase.featureMagnetic !== undefined) {
        targetUrl.searchParams.set("feature-magnetic", String(testCase.featureMagnetic));
      }
      ...
      if (!response?.ok()) {
        throw new Error(
          `Failed to load ${targetUrl.href} (status: ${response?.status()})`,
        );
      }
      ...
      // Wait for any final layout shifts after disabling transitions
      ...
      evidenceList.push({
        ...
        theme: testCase.theme,
        motionPreference: testCase.reducedMotion ? "reduced" : "full",
        reducedMotion: Boolean(testCase.reducedMotion),
        hasTouch: Boolean(testCase.hasTouch),
        browser: `Chromium ${browser.version()}`,
        frozenAnimations: true,
        mask: testCase.hoverSelector ? [testCase.hoverSelector] : [],
        featureFlags: {
    ...
    const report = {
      phase: 8,
      name: "Phase 8 Visual Evidence - Advanced Motion, Reveal and Magnetic Dynamics",
      browser: `Chromium ${browser.version()}`,
      commit,
    ...
    console.log(
      `Successfully generated ${evidenceList.length} Phase 8 evidence artifacts in ${OUTPUT_DIR}`,
    );
  } finally {
    await browser.close();
